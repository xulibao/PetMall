//
//  SAModalPresentationViewController.m
//  qmui
//
...
@property(nonatomic, weak, readwrite) SAModalPresentationViewController *qmui_modalPresentationViewController;

@end

@implementation SAModalPresentationViewController (UIAppearance)

static SAModalPresentationViewController *appearance;
...
+ (void)initDefaultAppearance {
    ...
    appearance.animationStyle = SAModalPresentationAnimationStyleFade;
    appearance.contentViewMargins = UIEdgeInsetsMake(20, 20, 20, 20);
    appearance.maximumContentViewWidth = (375 - UIEdgeInsetsGetHorizontalValue(appearance.contentViewMargins));
}
...
@interface SAModalPresentationViewController ()

@property(nonatomic, strong) SAModalPresentationWindow *containerWindow;
...
@implementation SAModalPresentationViewController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [SAModalPresentationViewController dimmedApplicationWindow];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [SAModalPresentationViewController resetDimmedApplicationWindow];
    ...
    void (^didHiddenCompletion)(BOOL finished) = ^(BOOL finished) {
        ...
        if (self.shownInWindowMode) {
            // 恢复 keyWindow 之前做一下检查，避免这个问题 https://github.com/SA/SA_iOS/issues/90
...
- (void)setContentViewController:(UIViewController<SAModalPresentationContentViewControllerProtocol> *)contentViewController {
...
- (void)showingAnimationWithCompletion:(void (^)(BOOL))completion {
    if (self.animationStyle == SAModalPresentationAnimationStyleFade) {
        self.dimmingView.alpha = 0.0;
        self.contentView.alpha = 0.0;
        [UIView animateWithDuration:.2 delay:0.0 options:SAViewAnimationOptionsCurveOut animations:^{
    ...
    } else if (self.animationStyle == SAModalPresentationAnimationStylePopup) {
        self.dimmingView.alpha = 0.0;
        self.contentView.transform = CGAffineTransformMakeScale(0, 0);
        [UIView animateWithDuration:.3 delay:0.0 options:SAViewAnimationOptionsCurveOut animations:^{
            self.dimmingView.alpha = 1.0;
    ...
    } else if (self.animationStyle == SAModalPresentationAnimationStyleSlide) {
        self.dimmingView.alpha = 0.0;
...
- (void)showWithAnimated:(BOOL)animated completion:(void (^)(BOOL))completion {
    ...
    if (!self.containerWindow) {
        self.containerWindow = [[SAModalPresentationWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
        self.containerWindow.windowLevel = UIWindowLevelAlert - 4.0;
        self.containerWindow.backgroundColor = UIColorClear;// 避免横竖屏旋转时出现黑色
    }
    self.supportedOrientationMask = [SAModalPresentationViewController visibleViewController].supportedInterfaceOrientations;
...
- (void)handleKeyboardWillShow:(NSNotification *)notification {
    CGFloat keyboardHeight = [SAModalPresentationViewController keyboardHeightWithNotification:notification inView:self.view];
    if (keyboardHeight > 0) {
...
- (BOOL)shouldAutorotate {
    UIViewController *visibleViewController = [SAModalPresentationViewController visibleViewController];
...
- (UIStatusBarStyle)preferredStatusBarStyle {
    ...
    UIViewController *visibleViewController = [SAModalPresentationViewController visibleViewController];
    if (visibleViewController != self && [visibleViewController respondsToSelector:@selector(preferredStatusBarStyle)]) {
...
@implementation SAModalPresentationViewController (Manager)

+ (BOOL)isAnyModalPresentationViewControllerVisible {
    for (UIWindow *window in [[UIApplication sharedApplication] windows]) {
        if ([window isKindOfClass:[SAModalPresentationWindow class]] && !window.hidden) {
            return YES;
        }
    }
    return NO;
...
+ (BOOL)hideAllVisibleModalPresentationViewControllerIfCan {
    ...
    for (UIWindow *window in [[UIApplication sharedApplication] windows]) {
        if (![window isKindOfClass:[SAModalPresentationWindow class]]) {
            continue;
        }
        ...
        SAModalPresentationViewController *modalViewController = (SAModalPresentationViewController *)window.rootViewController;
        BOOL canHide = YES;
...
@implementation SAModalPresentationWindow
...
@implementation UIViewController (SAModalPresentationViewController)

static char kAssociatedObjectKey_ModalPresentationViewController;
- (void)setQmui_modalPresentationViewController:(SAModalPresentationViewController *)modalPresentedViewController {
...
- (SAModalPresentationViewController *)qmui_modalPresentationViewController {
    return (SAModalPresentationViewController *)objc_getAssociatedObject(self, &kAssociatedObjectKey_ModalPresentationViewController);
}

- (UIViewController *)sa_visibleViewControllerIfExist {
    
    if (self.presentedViewController) {
        return [self.presentedViewController sa_visibleViewControllerIfExist];
    }
    
    if ([self isKindOfClass:[UINavigationController class]]) {
        return [((UINavigationController *)self).visibleViewController sa_visibleViewControllerIfExist];
    }
    
    if ([self isKindOfClass:[UITabBarController class]]) {
        return [((UITabBarController *)self).selectedViewController sa_visibleViewControllerIfExist];
    }
...
@implementation SAModalPresentationViewController(Helper)
...
+ (nullable UIViewController *)visibleViewController {
    ...
    UIViewController *visibleViewController = [rootViewController sa_visibleViewControllerIfExist];
    return visibleViewController;
}

+ (CGFloat)keyboardHeightWithNotification:(NSNotification *)notification {
    return [SAModalPresentationViewController keyboardHeightWithNotification:notification inView:nil];
}
