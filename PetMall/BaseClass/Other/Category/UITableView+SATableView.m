...
@implementation UITableView (SATableView)

+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
//        ReplaceMethod([self class], @selector(initWithFrame:style:), @selector(sa_initWithFrame:style:));
//        ReplaceMethod([self class], @selector(sizeThatFits:), @selector(sa_sizeThatFits:));
    });
}

- (instancetype)sa_initWithFrame:(CGRect)frame style:(UITableViewStyle)style {
    [self sa_initWithFrame:frame style:style];
...
- (CGSize)sa_sizeThatFits:(CGSize)size {
    [self alertEstimatedHeightUsageIfDetected];
    CGSize result = [self sa_sizeThatFits:size];
    return result;
}

- (NSIndexPath *)sa_indexPathForRowAtView:(UIView *)view {
    ...
    return [self sa_indexPathForRowAtView:view.superview];
}

- (NSInteger)sa_indexForSectionHeaderAtView:(UIView *)view {
    [self alertEstimatedHeightUsageIfDetected];
...
- (NSArray<NSNumber *> *)sa_indexForVisibleSectionHeaders {
    ...
    for (NSInteger i = 0; i < visibleSections.count; i++) {
        ...
        if ([self sa_isHeaderVisibleForSection:section]) {
...
- (NSInteger)sa_indexOfPinnedSectionHeader {
    NSArray<NSNumber *> *visibleSectionIndex = [self sa_indexForVisibleSectionHeaders];
    for (NSInteger i = 0; i < visibleSectionIndex.count; i++) {
        NSInteger section = visibleSectionIndex[i].integerValue;
        if ([self sa_isHeaderPinnedForSection:section]) {
            return section;
        } else {
            continue;
        }
...
- (BOOL)sa_isHeaderPinnedForSection:(NSInteger)section {
    ...
    BOOL isSectionScrollIntoContentInsetTop = self.contentOffset.y + self.sa_contentInset.top > CGRectGetMinY(rectForSection);// 表示这个 section 已经往上滚动，超过 contentInset.top 那条线了
    BOOL isSectionStayInContentInsetTop = self.contentOffset.y + self.sa_contentInset.top <= CGRectGetMaxY(rectForSection) - CGRectGetHeight(rectForHeader);// 表示这个 section 还没被完全滚走
    BOOL isPinned = isSectionScrollIntoContentInsetTop && isSectionStayInContentInsetTop;
    return isPinned;
}

- (BOOL)sa_isHeaderVisibleForSection:(NSInteger)section {
    if (self.style != UITableViewStylePlain) return NO;
    if (section >= [self numberOfSections]) return NO;
    ...
    BOOL isSectionStayInContentInsetTop = self.contentOffset.y + self.sa_contentInset.top < CGRectGetMaxY(rectForSection);// 表示这个 section 还没被完全滚走
    BOOL isVisible = isSectionScrollIntoBounds && isSectionStayInContentInsetTop;
    return isVisible;
}

- (SATableViewCellPosition)sa_positionForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSInteger numberOfRowsInSection = [self.dataSource tableView:self numberOfRowsInSection:indexPath.section];
    if (numberOfRowsInSection == 1) {
        return SATableViewCellPositionSingleInSection;
    }
    if (indexPath.row == 0) {
        return SATableViewCellPositionFirstInSection;
    }
    if (indexPath.row == numberOfRowsInSection - 1) {
        return SATableViewCellPositionLastInSection;
    }
    return SATableViewCellPositionMiddleInSection;
}

- (BOOL)sa_cellVisibleAtIndexPath:(NSIndexPath *)indexPath {
    NSArray<NSIndexPath *> *visibleCellIndexPaths = self.indexPathsForVisibleRows;
...
- (void)sa_scrollToRowFittingOffsetY:(CGFloat)offsetY atIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated {
    [self alertEstimatedHeightUsageIfDetected];
    ...
    if (canScrollRowToTop) {
        ...
        [self sa_scrollToBottomAnimated:animated];
    }
}

- (CGSize)sa_realContentSize {
    [self alertEstimatedHeightUsageIfDetected];
...
- (BOOL)sa_canScroll {
    ...
    if ([self.tableHeaderView isKindOfClass:[UISearchBar class]]) {
        BOOL canScroll = self.sa_realContentSize.height + UIEdgeInsetsGetVerticalValue(self.sa_contentInset) > CGRectGetHeight(self.bounds);
        return canScroll;
    } else {
        return [super sa_canScroll];
    }
}
