...
+ (CAShapeLayer *)sa_seperatorDashLayerWithLineLength:(NSInteger)lineLength
    ...
    if (isHorizontal) {
        ...
        CGPathAddLineToPoint(path, NULL, UI_SCREEN_WIDTH, lineWidth / 2);
    } else {
        CGPathMoveToPoint(path, NULL, lineWidth / 2, 0);
        CGPathAddLineToPoint(path, NULL, lineWidth / 2, UI_SCREEN_HEIGHT);
    }
    layer.path = path;
    CGPathRelease(path);
...
+ (CAShapeLayer *)sa_seperatorDashLayerInHorizontal {
    CAShapeLayer *layer = [CAShapeLayer sa_seperatorDashLayerWithLineLength:2 lineSpacing:2 lineWidth:PixelOne lineColor:kColorSeperateLine.CGColor isHorizontal:YES];
    return layer;
}
+ (CAShapeLayer *)sa_seperatorDashLayerInVertical {
    CAShapeLayer *layer = [CAShapeLayer sa_seperatorDashLayerWithLineLength:2 lineSpacing:2 lineWidth:PixelOne lineColor:kColorSeperateLine.CGColor isHorizontal:NO];
    return layer;
}

+ (CALayer *)sa_separatorLayer {
    CALayer *layer = [CALayer layer];
    [layer sa_removeDefaultAnimations];
    layer.backgroundColor = kColorSeperateLine.CGColor;
    layer.frame = CGRectMake(0, 0, 0, PixelOne);
    return layer;
}

+ (CALayer *)sa_separatorLayerForTableView {
    CALayer *layer = [self sa_separatorLayer];
    layer.backgroundColor = kColorSeperateLine.CGColor;
    return layer;
}
