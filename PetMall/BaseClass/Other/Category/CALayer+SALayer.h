 ...
 */
- (void)sa_sendSublayerToBack:(CALayer *)sublayer;
...
+ (CAShapeLayer *)sa_seperatorDashLayerWithLineLength:(NSInteger)lineLength
...
+ (CAShapeLayer *)sa_seperatorDashLayerInHorizontal;

+ (CAShapeLayer *)sa_seperatorDashLayerInVertical;

/**
 * 产生一个适用于做通用分隔线的 layer，高度为 PixelOne，默认会移除动画，并且背景色用 UIColorSeparator
 */
+ (CALayer *)sa_separatorLayer;

/**
 * 产生一个适用于做列表分隔线的 layer，高度为 PixelOne，默认会移除动画，并且背景色用 TableViewSeparatorColor
 */
+ (CALayer *)sa_separatorLayerForTableView;
