...
- (void)didInitialized {
    ...
    STEmptyView *appearance = [STEmptyView appearance];
    ...
    self.scrollView.contentInset = appearance.contentViewInsets;
    ...
    _loadingView = (UIView<STEmptyViewLoadingViewProtocol> *)[[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
    ...
    _actionButton = [SAGhostButton buttonWithType:UIButtonTypeSystem];
    self.actionButton.sa_outsideEdge = UIEdgeInsetsMake(-20, -20, -20, -20);
...
- (void)updateDetailTextLabelWithText:(NSString *)text {
    if (self.detailTextLabelFont && self.detailTextLabelTextColor && text) {
        NSAttributedString *string = [[NSAttributedString alloc] initWithString:text attributes:@{
                                                                                                  ...
                                                                                                  NSParagraphStyleAttributeName: [NSMutableParagraphStyle sa_paragraphStyleWithLineHeight:self.detailTextLabelFont.pointSize + 10 lineBreakMode:NSLineBreakByWordWrapping textAlignment:NSTextAlignmentCenter]
                                                                                                  }];
...
- (void)setLoadingView:(UIView<STEmptyViewLoadingViewProtocol> *)loadingView {
...
@interface STEmptyView (UIAppearance)

@end

@implementation STEmptyView (UIAppearance)
...
+ (void)setDefaultAppearance {
    STEmptyView *appearance = [STEmptyView appearance];
    appearance.contentViewInsets = UIEdgeInsetsMake(0, 10, 0, 10); // 避免 label 直接撑满到屏幕两边，不好看
