...
const CGSize SAFloatLayoutViewAutomaticalMaximumItemSize = {-1, -1};

@implementation SAFloatLayoutView
...
- (void)didInitialized {
    ...
    self.maximumItemSize = SAFloatLayoutViewAutomaticalMaximumItemSize;
}
...
- (CGSize)layoutSubviewsWithSize:(CGSize)size shouldLayout:(BOOL)shouldLayout {
    ...
    CGSize maximumItemSize = CGSizeEqualToSize(self.maximumItemSize, SAFloatLayoutViewAutomaticalMaximumItemSize) ? CGSizeMake(size.width - UIEdgeInsetsGetHorizontalValue(self.padding), size.height - UIEdgeInsetsGetVerticalValue(self.padding)) : self.maximumItemSize;
...
- (CGSize)intrinsicContentSize {
    return [self sizeThatFits:(CGSize){UI_SCREEN_WIDTH, CGFLOAT_MAX}];
}
