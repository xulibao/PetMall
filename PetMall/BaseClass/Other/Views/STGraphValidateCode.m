...
#import "STGraphValidateCode.h"

@implementation STGraphValidateCode
...
//    self.layer.cornerRadius = 4;
    ...
    _maxLineNumbers = 6;
    UITapGestureRecognizer *tapgesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeCode)];
    [self addGestureRecognizer:tapgesture];
    tapgesture = nil;
    
    
    
    [self generateValidateCode];
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i= 0; i <_validateCode.length; i ++) {
        NSRange  range = NSMakeRange(i, 1);
        NSString * chars = [_validateCode substringWithRange:range];
...
{
    __autoreleasing NSMutableArray *numbers = [NSMutableArray array];
