...
- (void)layoutSubviews {
    ...
    if (self.fadeLeftLayer) {
        ...
        [self.layer sa_bringSublayerToFront:self.fadeLeftLayer];// 显示非英文字符时，UILabel 内部会额外多出一层 layer 盖住了这里的 fadeLayer，所以要手动提到最前面
    }
    if (self.fadeRightLayer) {
        ...
        [self.layer sa_bringSublayerToFront:self.fadeRightLayer];// 显示非英文字符时，UILabel 内部会额外多出一层 layer 盖住了这里的 fadeLayer，所以要手动提到最前面
    }
}
...
@implementation SAMarqueeLabel (ReusableView)
