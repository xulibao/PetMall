...
#import "UIButton+SAButton.h"
#import "NSParagraphStyle+SAParagraphStyle.h"
#import "STCGUtilities.h"

// 按钮
#pragma mark - UIButton
#define ButtonHighlightedAlpha          (1)           // 按钮Highlighted状态的透明度
#define ButtonDisabledAlpha             (0.5f)              // 按钮Disabled状态的透明度
#define ButtonTintColor                 (kColorBGRed)                  // 普通按钮的颜色
...
- (void)didInitialized {
    ...
    self.imagePosition = SAButtonImagePositionLeft;
}
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    switch (self.imagePosition) {
        case SAButtonImagePositionTop:
        case SAButtonImagePositionBottom: {
        ...
        case SAButtonImagePositionLeft:
        case SAButtonImagePositionRight: {
            // 图片和文字水平排版时，高度以文字或图片的最大高度为最终高度
            // 注意这里有一个和系统不一致的行为：当 titleLabel 为多行时，系统的 sizeThatFits: 计算结果固定是单行的，所以当 SAButtonImagePositionLeft 并且titleLabel 多行的情况下，SAButton 计算的结果与系统不一致
...
- (void)layoutSubviews {
    ...
    if (self.imagePosition == SAButtonImagePositionTop || self.imagePosition == SAButtonImagePositionBottom) {
        ...
        if (self.imagePosition == SAButtonImagePositionTop) {
    ...
    } else if (self.imagePosition == SAButtonImagePositionLeft || self.imagePosition == SAButtonImagePositionRight) {
        ...
        if (self.imagePosition == SAButtonImagePositionLeft) {
            switch (self.contentHorizontalAlignment) {
                case UIControlContentHorizontalAlignmentLeft:
                    imageFrame = isImageViewShowing ? CGRectSetX(imageFrame, contentEdgeInsets.left + self.imageEdgeInsets.left) : imageFrame;
...
- (void)setImagePosition:(SAButtonImagePosition)imagePosition {
...
- (void)adjustsButtonHighlighted {
    if (self.highlightedBackgroundColor) {
        if (!self.highlightedBackgroundLayer) {
            ...
            [self.highlightedBackgroundLayer sa_removeDefaultAnimations];
...
@interface SALinkButton ()

@property(nonatomic, strong) CALayer *underlineLayer;
@end

@implementation SALinkButton
...
- (void)didInitialized {
    ...
    [self.underlineLayer sa_removeDefaultAnimations];
...
const CGFloat SAGhostButtonCornerRadiusAdjustsBounds = -1;

@implementation SAGhostButton

- (instancetype)initWithFrame:(CGRect)frame {
    return [self initWithGhostColor:ButtonTintColor];
}
...
//- (instancetype)initWithGhostType:(SAGhostButtonColor)ghostType frame:(CGRect)frame {
//    UIColor *ghostColor = nil;
//    switch (ghostType) {
//        case SAGhostButtonColorBlue:
//            ghostColor = GhostButtonColorBlue;
//            break;
//        case SAGhostButtonColorRed:
//            ghostColor = GhostButtonColorRed;
//            break;
//        case SAGhostButtonColorGreen:
//            ghostColor = GhostButtonColorGreen;
//            break;
//        case SAGhostButtonColorGray:
//            ghostColor = GhostButtonColorGray;
//            break;
//        case SAGhostButtonColorWhite:
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    if (self = [super initWithCoder:aDecoder]) {
        [self initializeWithGhostColor:ButtonTintColor];
    }
    return self;
}
...
- (void)layoutSublayersOfLayer:(CALayer *)layer {
    ...
    if (self.cornerRadius != SAGhostButtonCornerRadiusAdjustsBounds) {
...
@interface SAGhostButton (UIAppearance)

@end

@implementation SAGhostButton (UIAppearance)
...
+ (void)setDefaultAppearance {
    SAGhostButton *appearance = [SAGhostButton appearance];
    appearance.borderWidth = 1;
    appearance.cornerRadius = SAGhostButtonCornerRadiusAdjustsBounds;
...
const CGFloat SAFillButtonCornerRadiusAdjustsBounds = -1;

@implementation SAFillButton
...
- (instancetype)initWithFrame:(CGRect)frame {
    return [self initWithFillColor:kColorBG titleTextColor:ButtonTintColor];
}

//- (instancetype)initWithFrame:(CGRect)frame {
//    return [self initWithFillType:SAFillButtonColorBlue frame:frame];
//}
//
...
//    switch (fillType) {
//        case SAFillButtonColorBlue:
//            fillColor = FillButtonColorBlue;
//            break;
//        case SAFillButtonColorRed:
//            fillColor = FillButtonColorRed;
//            break;
//        case SAFillButtonColorGreen:
//            fillColor = FillButtonColorGreen;
//            break;
//        case SAFillButtonColorGray:
//            fillColor = FillButtonColorGray;
//            break;
//        case SAFillButtonColorWhite:
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    if (self = [super initWithCoder:aDecoder]) {
        self.fillColor = kColorBG;
        self.titleTextColor = ButtonTintColor;
    }
    return self;
}
...
- (void)layoutSublayersOfLayer:(CALayer *)layer {
    [super layoutSublayersOfLayer:layer];
    if (self.cornerRadius != SAFillButtonCornerRadiusAdjustsBounds) {
        self.layer.cornerRadius = self.cornerRadius;
    } else {
        self.layer.cornerRadius = flat(CGRectGetHeight(self.bounds) / 2);
    }
...
@interface SAFillButton (UIAppearance)

@end

@implementation SAFillButton (UIAppearance)
...
+ (void)setDefaultAppearance {
    SAFillButton *appearance = [SAFillButton appearance];
    appearance.cornerRadius = SAFillButtonCornerRadiusAdjustsBounds;
