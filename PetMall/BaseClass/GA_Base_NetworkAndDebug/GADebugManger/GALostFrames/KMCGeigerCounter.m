...
- (void)updateMeterLabel
{
    ...
    if (droppedFrameCount <= 0) {
        ...
        droppedString = [NSString stringWithFormat:@"丢掉%ld", (long) droppedFrameCount];
    }

    if (drawnFrameCount == -1) {
        drawnString = @"-";
    } else {
        drawnString = [NSString stringWithFormat:@"完成渲染%ld", (long) drawnFrameCount];
    }
...
- (void)enable
{
    ...
    _window.rootViewController = [GATempViewController sharedInstance];

    CGFloat const kMeterWidth = 110.0;
    CGFloat xOrigin = 0.0;
    ...
    [self.meterLabel adjustsFontSizeToFitWidth];
